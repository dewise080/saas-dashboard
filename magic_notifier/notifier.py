...
from magic_notifier.services import log_notification_event
...
def notify(
    ...
    recv_count = _receiver_count(receivers)
        ...
        try:
            if via == "email":
                ...
                log_notification_event(
                    ...
                    metadata={"receivers_count": recv_count, "subject": subject},
                )
                ...
                log_notification_event(
                    ...
                    metadata={"receivers_count": recv_count},
                )
            ...
            log_notification_event(
                channel=via,
                status="failed",
                gateway=email_gateway if via == "email" else sms_gateway if via == "sms" else whatsapp_gateway if via == "whatsapp" else telegram_gateway if via == "telegram" else push_gateway if via == "push" else None,
                provider=email_gateway if via == "email" else sms_gateway if via == "sms" else whatsapp_gateway if via == "whatsapp" else telegram_gateway if via == "telegram" else push_gateway if via == "push" else None,
                metadata={"receivers_count": recv_count},
